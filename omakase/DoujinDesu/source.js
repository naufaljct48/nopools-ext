...
var _Sources = (() => {
  ...
  var BASE_URL = "https://doujin.desu.xxx";
  var API_URL = `${BASE_URL}/api`;
  var APP_SECRET = "dfdf72051dbfdc7d76889ebd31324e74";
  var KEY_SALT = "doujindesu-scrapers-cannot-read-this-super-secret-salt-2026-v2";
  var KEY_WINDOW_MS = 36e5;
  var deviceId = "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, (c) => {
  ...
  var parseMangaDetails = (data, mangaId) => {
    ...
    if (altTitles) {
      for (const alt of altTitles.split("|").map((x) => x.trim()).filter(Boolean)) {
        titles.push(alt);
      }
    }
    const genres = [];
  ...
  var parseChapterList = (chapters, mangaId) => {
    ...
    for (const ch of sorted) {
      ...
      result.push(App.createChapter({
        ...
        langCode: LANG,
        time: ch?.created_at ? new Date(ch.created_at) : /* @__PURE__ */ new Date(),
        sortingIndex
      }));
      sortingIndex--;
    }
  ...
  var DoujinDesuInfo = {
    ...
    sourceTags: [
      { text: "Indonesian", type: import_types.BadgeColor.GREY },
      { text: "18+", type: import_types.BadgeColor.RED }
    ],
  ...
  var DoujinDesu = class extends import_types.Source {
    constructor() {
      super(...arguments);
      this.requestManager = App.createRequestManager({
        ...
        interceptor: {
          interceptRequest: async (request) => {
            const isImage = /\.(png|jpe?g|webp|gif|avif)(\?|$)/i.test(request.url) || request.url.includes("desu.xxx/content/") || request.url.includes("desu.pics") || request.url.includes("cdnasu");
...
Please open ${BASE_URL} with the cloud icon first.`);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      return App.createPagedResults({
        results,
        metadata: results.length >= limit ? { page: page + 1 } : void 0
      });
    }
    async getSearchTags() {
    ...
    async getSearchResults(query, metadata) {
      ...
      const genre = (query.includedTags ?? []).map((t) => String(t.id ?? "")).find((id) => id.startsWith("genre:"))?.replace("genre:", "") ?? "";
      const parts = [];
