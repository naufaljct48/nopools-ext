...
/**
 * The gh-pages README is a deploy artifact, not a hand-maintained file. Patching only
 * the marker block left its header advertising whichever branch first created it —
 * which is how it kept pointing at `mangastream` after the rename. Rewrite it whole.
 */
function buildRootReadme(status) {
    const folderName = path.basename(path.resolve(bundleFolder))
    const label = titleCase(folderName)

    return `# 📚 Nopools ${label} (0.8)
> A curated Paperback (0.8) source collection by [NaufalJCT48](https://github.com/${GITHUB_USER})
...
| Repository | Base URL |
|------------|----------|
| \`${folderName}\` (active) | ${PAGES_ROOT}/${folderName} |${notice?.movedTo ? '' : ''}
...
- Report issues via [GitHub Issues](${REPO_URL}/issues).

## 📜 License
GPL-3.0 — see [LICENSE](${REPO_URL}/blob/${folderName}/LICENSE).
`
}
...
;(async () => {
    ...
    // The source-branch README is hand-written outside the markers, so only the table
    // is replaced there. The gh-pages one is regenerated whole (see buildRootReadme).
    console.log(patchReadme('./README.md', status) ? 'synced ./README.md' : `skipped ./README.md (no ${BEGIN} markers)`)

    if (extraReadme && !notice?.movedTo) {
        fs.writeFileSync(extraReadme, buildRootReadme(status))
        console.log(`rewrote ${extraReadme}`)
    } else if (extraReadme) {
        // A deprecated branch must never overwrite the published README with its own
        // stale source table — the active branch owns that file
        console.log(`skipped ${extraReadme} (this branch is deprecated)`)
    }
