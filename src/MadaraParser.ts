...
export class Parser {
  async parseMangaDetails(
    ...
    const title: string = this.decodeHTMLEntity(
    ...
    const author: string = this.decodeHTMLEntity(
      $("div.author-content").first().text().replace("\\n", "").trim()
    ).replace("Updating", "");
    const artist: string = this.decodeHTMLEntity(
      $("div.artist-content").first().text().replace("\\n", "").trim()
    ).replace("Updating", "");
    const description: string = this.decodeHTMLEntity(
      $("div.description-summary").first().text()
    )
      .replace("Show more", "")
      .trim();
  ...
  parseChapterList(
    ...
    for (const obj of $("li.wp-manga-chapter  ").toArray()) {
      ...
      if (typeof timeSelector !== "undefined") {
        ...
        mangaTime = this.parseDate(
          $("span.chapter-release-date > i", obj).text().trim()
        );
      }
  ...
  async parseChapterDetails(
    ...
    for (const obj of $(selector).toArray()) {
      ...
      if (!page) {
        throw new Error(
          `Could not parse page for postId:${mangaId} chapterId:${chapterId}`
        );
      }
