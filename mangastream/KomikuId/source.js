...
var _Sources = (() => {
  ...
  // src/KomikuId/KomikuId.ts
  var KomikuId_exports = {};
  __export(KomikuId_exports, {
    KomikuId: () => KomikuId,
    KomikuIdInfo: () => KomikuIdInfo
  });
  ...
  var MangaStreamParser = class {
    ...
    async parseSearchResults($2, source) {
      ...
      for (const obj of $2("div.bs", "div.listupd").toArray()) {
        ...
        const subtitle = $2("div.epxs", obj).text().trim();
        results.push({
          slug,
          path,
  ...
  function getIncludedTagBySection2(section, tags) {
...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`);
  ...
  // src/KomikuId/KomikuIdParser.ts
  var KomikuIdParser = class extends MangaStreamParser {
    parseTags($2) {
      const sections = [];
      const orderTags = [];
      $2('select[name="orderby"] option').each((_, el) => {
      ...
      const genreTags = [];
      $2('select[name="genre"] option').each((_, el) => {
        const value = $2(el).attr("value");
        const label = $2(el).text().trim();
      ...
      const statusTags = [];
      $2('select[name="status"] option').each((_, el) => {
        const value = $2(el).attr("value");
        const label = $2(el).text().trim();
        if (value) {
      ...
      const typeTags = [];
      $2('select[name="category_name"] option').each((_, el) => {
        const value = $2(el).attr("value");
        const label = $2(el).text().trim();
        if (value) {
    ...
    parseChapterList($2, mangaId) {
      const chapters = [];
      for (const chapter of $2("#Daftar_Chapter tbody tr").toArray()) {
        if ($2(chapter).find("th").length > 0) continue;
        const $link = $2("td.judulseries a", chapter);
        const title = $link.find("span").text().trim();
        const id = this.idCleaner($link.attr("href") ?? "");
        const views = $2("td.pembaca i", chapter).text().trim();
        const date = $2("td.tanggalseries", chapter).text().trim();
    ...
    parseChapterDetails($2, mangaId, chapterId) {
      const pages = [];
      for (const img of $2("#Baca_Komik img.ww").toArray()) {
        const src = $2(img).attr("src");
        if (!src) continue;
        pages.push(src);
      }
      const prevChapter = $2("div.nxpr a:first-child").attr("href");
      const nextChapter = $2("div.nxpr a:last-child").attr("href");
  ...
  var KomikuIdInfo = {
    ...
    intents: import_types4.SourceIntents.MANGA_CHAPTERS | import_types4.SourceIntents.HOMEPAGE_SECTIONS | import_types4.SourceIntents.CLOUDFLARE_BYPASS_REQUIRED,
    sourceTags: [
      {
        text: "Indonesia",
  ...
  var KomikuId = class extends MangaStream {
    constructor() {
      super(...arguments);
      this.baseUrl = DOMAIN;
      this.directoryPath = "manga";
      this.usePostIds = false;
      this.parser = new KomikuIdParser();
    }
    configureSections() {
      this.homescreen_sections["latest_update"].enabled = true;
      this.homescreen_sections["latest_update"].selectorFunc = ($2) => $2(".bge");
      this.homescreen_sections["latest_update"].titleSelectorFunc = ($2, element) => $2("div.kan h3", element).text().trim();
      this.homescreen_sections["latest_update"].subtitleSelectorFunc = ($2, element) => {
        const latestChap = $2("div.new1:last-child span:last-child", element).text().trim();
        return latestChap;
      };
      this.homescreen_sections["latest_update"].getViewMoreItemsFunc = (page) => `pustaka/?orderby=modified&category_name=&genre=&genre2=&status=&page=${page}`;
      this.homescreen_sections["top_alltime"].enabled = true;
      this.homescreen_sections["top_alltime"].selectorFunc = ($2) => $2(".bge");
      this.homescreen_sections["top_alltime"].titleSelectorFunc = ($2, element) => $2("div.kan h3", element).text().trim();
      this.homescreen_sections["top_alltime"].subtitleSelectorFunc = ($2, element) => {
        const latestChap = $2("div.new1:last-child span:last-child", element).text().trim();
        return latestChap;
      };
      this.homescreen_sections["top_alltime"].getViewMoreItemsFunc = (page) => `pustaka/?orderby=meta_value_num&category_name=&genre=&genre2=&status=&page=${page}`;
    ...
    async constructSearchRequest(page, query) {
      ...
      const url = page === 1 ? `${this.baseUrl}/pustaka/?${params.toString()}` : `https://api.komiku.id/manga/page/${page}/?${params.toString()}`;
      return App.createRequest({
        url,
        method: "GET"
      });
    ...
    async parseViewMore($2) {
      const manga = [];
      for (const item of $2(".bge").toArray()) {
        const title = $2("div.kan h3", item).text().trim();
        const image = $2("div.bgei img", item).attr("src") ?? "";
        const id = this.parser.idCleaner($2("div.bgei a", item).attr("href") ?? "");
        const subtitle = $2("div.new1:last-child span:last-child", item).text().trim();
        if (!id || !title) continue;
        manga.push(App.createPartialSourceManga({
          id,
          image,
    ...
    async getSearchResults(query, metadata) {
      ...
      if (query.title) {
        ...
        request = App.createRequest({
          url: page === 1 ? `${this.baseUrl}/pustaka/?${params.toString()}` : `https://api.komiku.id/manga/page/${page}/?${params.toString()}`,
          method: "GET"
        });
      }
      const response = await this.requestManager.schedule(request, 1);
  ...
  return __toCommonJS(KomikuId_exports);
})();
