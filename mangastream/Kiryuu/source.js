...
var _Sources = (() => {
  ...
  var WEBSITE_BASE = "https://v5.kiryuu.to";
  ...
  var extractText = (html3, regex) => {
    const match = html3.match(regex);
    return match?.[1]?.trim() ?? "";
  };
  var parseMangaDetails = (html3, mangaId) => {
    const $2 = cheerio.load(html3);
    const titles = [];
    const mainTitle = $2('h1[itemprop="name"]').first().text().trim() || extractText(html3, /<h1[^>]*itemprop=["']name["'][^>]*>(.*?)<\/h1>/is);
    if (mainTitle) titles.push(decodeHTMLEntity(mainTitle.replace(/<[^>]*>/g, "")));
    const altTitle = $2('h1[itemprop="name"]').first().next("div").text().trim();
    if (altTitle) {
      const altTitles = altTitle.split(",").map((t) => t.trim()).filter((t) => t);
      titles.push(...altTitles.map((t) => decodeHTMLEntity(t.replace(/<[^>]*>/g, ""))));
    }
    const image = normalizeUrl($2('[itemprop="image"] img').first().attr("src") ?? $2("img.wp-post-image").first().attr("src") ?? "");
    const statusText = html3.toLowerCase();
    const status = statusText.includes("ongoing") ? "Ongoing" : statusText.includes("completed") ? "Completed" : "Unknown";
    let author = "Unknown";
    $2("h4").each((_, element) => {
      const label = $2(element).text().toLowerCase();
      if (label.includes("author") || label.includes("artist")) {
        const value = $2(element).parent().find("p").first().text().trim();
        if (value) author = value;
      }
    });
    const arrayTags = [];
    $2('a[itemprop="genre"]').each((_, element) => {
      const label = $2(element).text().trim();
      const id = ($2(element).attr("href") ?? "").match(/\/genre\/([^/]+)\/?/)?.[1] ?? "";
    ...
    let desc = $2('[itemprop="description"][data-show="true"]').first().text().trim();
    if (!desc) desc = $2('[itemprop="description"]').last().text().trim();
  ...
  var parseChapterList = (html3, mangaId) => {
    const $2 = cheerio.load(html3);
    const chapters = [];
    let sortingIndex = 0;
    $2("#chapter-list div[data-chapter-number]").each((_, element) => {
  ...
  var parseChapterDetails = (html3, mangaId, chapterId) => {
    const $2 = cheerio.load(html3);
    const pages = [];
    $2("section[data-image-data] img").each((_, element) => {
  ...
  var parseMangaList = (html3) => {
    const $2 = cheerio.load(html3);
    const results = [];
    const seen = /* @__PURE__ */ new Set();
    $2('a[href*="/manga/"]').each((_, element) => {
      ...
      const cardTitle = $card.find(`a[href$="/manga/${mangaId}/"]`).filter((_2, titleElement) => $2(titleElement).text().trim().length > 0).first().text().trim();
