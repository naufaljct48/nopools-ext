...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($2, mangaId) => {
    ...
    const altTitle = $2(".text-sm.text-text.line-clamp-1").first().text().trim();
    if (altTitle) {
    ...
    let author = "Unknown";
    $2(".flex.sm\\:justify-between.justify-start.items-center.gap-2").each((_, elem) => {
      const label = $2("h4", elem).text().toLowerCase();
      if (label.includes("author")) {
        author = $2(".inline p", elem).text().trim() || "Unknown";
      }
    });
    const arrayTags = [];
    $2('a[itemprop="genre"]').each((_, elem) => {
      const label = $2("span", elem).text().trim();
    ...
    let desc = $2('div[itemprop="description"][data-show="true"]').first().text().trim();
    if (!desc) {
      desc = $2('div[itemprop="description"][data-show="false"]').first().text().trim();
    }
    let type = "";
    let released = "";
    let views = "";
    let rating = "";
    $2(".space-y-2 .flex.sm\\:justify-between.justify-start.items-center.gap-2").each((_, elem) => {
      const label = $2("h4", elem).text().toLowerCase();
      const value = $2(".inline p, .inline", elem).text().trim();
...
${additionalInfo.join(" \u2022 ")}`;
  ...
  var parseChapterList = ($2, mangaId) => {
    const chapters = [];
    let sortingIndex = 0;
    $2("div[data-chapter-number]").each((_, elem) => {
      const $elem = $2(elem);
      const link = $2("a", $elem).first();
      const chapterUrl = link.attr("href") ?? "";
      const chapterId = chapterUrl.split("/").filter((x) => x).pop() ?? "";
      if (!chapterId) return;
      const chapNumStr = $elem.attr("data-chapter-number") ?? "0";
      const chapNum = parseFloat(chapNumStr) || 0;
      const title = $2(".font-medium.text-base span", $elem).first().text().trim();
      const name = title || `Chapter ${chapNumStr}`;
      const timeStr = $2("time", $elem).attr("datetime") ?? $2("time", $elem).text().trim();
  ...
  var parseChapterDetails = ($2, mangaId, chapterId) => {
    const pages = [];
    $2("section[data-image-data] img").each((_, elem) => {
  ...
  var parseMangaList = ($2) => {
    const results = [];
    $2(".swiper-slide.manga-swipe, .swiper-slide").each((_, elem) => {
      const $elem = $2(elem);
      const link = $2("a", $elem).first();
      const href = link.attr("href") ?? "";
      const mangaId = href.split("/").filter((x) => x).pop() ?? "";
      if (!mangaId) return;
      const title = link.attr("title") ?? $2("a", $elem).attr("title") ?? "";
      const image = normalizeUrl($2("img", $elem).first().attr("src") ?? "");
      let subtitle = "";
      const chapterText = $2(".text-sm.text-gray-300", $elem).first().text().trim();
      if (chapterText) {
    ...
    $2(".flex.flex-col.justify-between.px-4.py-1\\.5").each((_, elem) => {
      ...
      const image = normalizeUrl($2("img", $elem.parent()).first().attr("src") ?? "");
      const chapterText = $2(".text-sm.text-gray-300", $elem).first().text().trim();
  ...
  var parseSearchTags = ($2) => {
    ...
    const types = [];
    const statuses = [];
    $2("button[data-genre]").each((_, elem) => {
      const $elem = $2(elem);
    ...
    if (genres.length > 0) {
      sections.push(App.createTagSection({
        id: "genre",
        label: "Genres",
        tags: genres.map((x) => App.createTag(x))
      }));
    }
    if (types.length > 0) {
      sections.push(App.createTagSection({
    ...
    if (statuses.length > 0) {
      sections.push(App.createTagSection({
        id: "status",
        label: "Status",
        tags: statuses.map((x) => App.createTag(x))
      }));
    }
    return sections;
  };
