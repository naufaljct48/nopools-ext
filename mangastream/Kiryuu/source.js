...
var _Sources = (() => {
  ...
  var parseMangaDetails = (doc, mangaId) => {
    const titles = [];
    ...
    let author = "Unknown";
    $$(doc, ".flex.sm\\:justify-between.justify-start.items-center.gap-2").forEach((elem) => {
    ...
    const arrayTags = [];
    $$(doc, 'a[itemprop="genre"]').forEach((elem) => {
    ...
    let views = "";
    let rating = "";
    $$(doc, ".space-y-2 .flex.sm\\:justify-between.justify-start.items-center.gap-2").forEach((elem) => {
      const label = text(elem.querySelector("h4")).toLowerCase();
...
${additionalInfo.join(" \u2022 ")}`;
  ...
  var parseChapterList = (doc, mangaId) => {
    const chapters = [];
    let sortingIndex = 0;
    $$(doc, "div[data-chapter-number]").forEach((elem) => {
  ...
  var parseChapterDetails = (doc, mangaId, chapterId) => {
    const pages = [];
    $$(doc, "section[data-image-data] img").forEach((elem) => {
  ...
  var parseMangaList = (doc) => {
    const results = [];
    $$(doc, ".swiper-slide.manga-swipe, .swiper-slide").forEach((elem) => {
      const link = elem.querySelector("a");
    ...
    $$(doc, "#searchResults a").forEach((elem) => {
      const href = attr(elem, "href");
      const mangaId = href.split("/").filter((x) => x).pop() ?? "";
      if (!mangaId) return;
    ...
    $$(doc, ".flex.flex-col.justify-between.px-4.py-1\\.5").forEach((elem) => {
  ...
  var parseSearchTags = (doc) => {
    const genres = [];
    const types = [];
    const statuses = [];
    $$(doc, "button[data-genre]").forEach((elem) => {
    ...
    $$(doc, "button[data-type]").forEach((elem) => {
  ...
  var Kiryuu = class extends import_types.Source {
    constructor() {
      super(...arguments);
