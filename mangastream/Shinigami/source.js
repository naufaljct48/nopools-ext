...
var _Sources = (() => {
  ...
  var Shinigami_exports = {};
  __export(Shinigami_exports, {
    Shinigami: () => Shinigami,
...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`);
  ...
  var ShinigamiParser = class extends MangaStreamParser {
    parseMangaDetails($2, mangaId, source) {
      const result = JSON.parse($2.html());
      const data2 = result.data;
      const tags = [];
      if (data2.taxonomy.Genre) {
        data2.taxonomy.Genre.forEach((genre) => {
      ...
      return App.createSourceManga({
        ...
        mangaInfo: App.createMangaInfo({
          titles: [data2.title ?? ""],
          image: data2.thumbnail ?? "",
          status: data2.status === 1 ? "Ongoing" : "Completed",
          author: data2.taxonomy.Author?.map((x) => x.name).join(", ") ?? "Unknown",
          artist: data2.taxonomy.Artist?.map((x) => x.name).join(", ") ?? "Unknown",
    ...
    parseChapterDetails($2, mangaId, chapterId) {
      const result = JSON.parse($2.html());
      const pages = result.pageList.chapterPage.pages.filter((page) => !page.startsWith("999-")).map((page) => {
  ...
  // src/Shinigami/Shinigami.ts
  var DOMAIN = "https://app.shinigami.asia";
  var API_DOMAIN = "https://api.shngm.io";
  ...
  var Shinigami = class extends MangaStream {
    constructor() {
      super(...arguments);
      this.baseUrl = DOMAIN;
      this.parser = new ShinigamiParser();
    }
    async getRequest(url, method = "GET", param) {
      const request = App.createRequest({
        url: url + (param ?? ""),
        method,
        headers: {
    ...
    configureSections() {
      ...
      this.homescreen_sections.popular_today.getViewMoreItemsFunc = (page) => `${API_DOMAIN}/${API_BASE_PATH}/manga/list?page=${page}&page_size=30&sort=popularity`;
      this.homescreen_sections.latest_update.section = App.createHomeSection({
        id: "latest",
        title: "Latest Updates",
        containsMoreItems: true
      });
      this.homescreen_sections.latest_update.getViewMoreItemsFunc = (page) => `${API_DOMAIN}/${API_BASE_PATH}/manga/list?page=${page}&page_size=30&sort=latest`;
