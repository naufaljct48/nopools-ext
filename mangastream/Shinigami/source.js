...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`);
  ...
  var Shinigami = class extends MangaStream {
        ...
        interceptor: {
          interceptRequest: async (request) => {
            ...
            request.headers = {
              ...
              "Accept": isImageRequest ? "image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8" : "application/json"
            };
            return request;
          },
          interceptResponse: async (response) => {
    ...
    async getSearchResults(query, metadata) {
      ...
      metadata = result.meta.page < result.meta.total_page ? { page: page + 1 } : void 0;
      return App.createPagedResults({
        results: manga,
        metadata
      });
