...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`);
  ...
  var WestManga = class extends MangaStream {
    ...
    configureSections() {
      ...
      this.homescreen_sections["popular_today"].selectorFunc = ($2, element) => $2("div.bsx", $2("h2:contains(Komik Popular Hari Ini...)")?.parent()?.next());
      this.homescreen_sections["latest_update"].selectorFunc = ($2, element) => $2("div.bsx", $2("h2:contains(UPDATE KOMIK LAINYA...)")?.parent()?.next());
      this.homescreen_sections["latest_update"].subtitleSelectorFunc = ($2, element) => $2("div.epxs", element).text().trim();
    }
  };
