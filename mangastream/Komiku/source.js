...
var _Sources = (() => {
  ...
  var Komiku_exports = {};
  __export(Komiku_exports, {
    Komiku: () => Komiku,
    KomikuInfo: () => KomikuInfo
  });
  var import_types = __toESM(require_lib());

  // src/Komiku/KomikuHelper.ts
  var BASE_URL = "https://komiku.org";
  var API_URL = "https://api.komiku.org";
  var createRequestObject = (requestObj) => {
    const url = requestObj.url || "";
    const isImage = /\.(png|jpe?g|webp|gif|avif)$/i.test(url) || url.includes("thumbnail.komiku.org") || url.includes("img.komiku.org");
  ...
  var convertRelativeDate = (timeStr) => {
    const now = /* @__PURE__ */ new Date();
    const s = timeStr.toLowerCase().trim();
    if (/baru saja|just now/i.test(s)) return now;
    const m = s.match(/(\d+)\s*(menit|minute|min)/);
    if (m?.[1]) return new Date(now.getTime() - parseInt(m[1]) * 60 * 1e3);
    const h = s.match(/(\d+)\s*(jam|hour|hr)/);
    if (h?.[1]) return new Date(now.getTime() - parseInt(h[1]) * 3600 * 1e3);
    const d = s.match(/(\d+)\s*(hari|day)/);
    if (d?.[1]) return new Date(now.getTime() - parseInt(d[1]) * 86400 * 1e3);
    const w = s.match(/(\d+)\s*(minggu|week)/);
    if (w?.[1]) return new Date(now.getTime() - parseInt(w[1]) * 7 * 86400 * 1e3);
    const mo = s.match(/(\d+)\s*(bulan|month)/);
    if (mo?.[1]) return new Date(now.getTime() - parseInt(mo[1]) * 30 * 86400 * 1e3);
    const y = s.match(/(\d+)\s*(tahun|year)/);
    if (y?.[1]) return new Date(now.getTime() - parseInt(y[1]) * 365 * 86400 * 1e3);
    const dmy = s.match(/(\d{1,2})\/(\d{1,2})\/(\d{4})/);
    if (dmy?.[1] && dmy[2] && dmy[3]) return new Date(parseInt(dmy[3]), parseInt(dmy[2]) - 1, parseInt(dmy[1]));
    return now;
  };

  // src/Komiku/KomikuParser.ts
  var extractText = (html, regex) => (html.match(regex)?.[1] ?? "").trim();
  var extractAll = (html, regex) => Array.from(html.matchAll(regex));
  ...
  var parseMangaDetails = (html, mangaId) => {
    ...
    const tagSections = tags.length > 0 ? [App.createTagSection({ id: "genres", label: "Genres", tags })] : [];
  ...
  var getTagValue = (query, sectionId) => {
  ...
  var Komiku = class extends import_types.Source {
    constructor() {
      super(...arguments);
  ...
  return __toCommonJS(Komiku_exports);
})();
