...
var _Sources = (() => {
  ...
  // src/Comick/ComickHelper.ts
  var DOMAINS = [
    "https://comick.live",
    "https://comick.art"
  ];
  var DEFAULT_DOMAIN = DOMAINS[0];
  ...
  var createRequestObject = (options) => {
    ...
    return App.createRequest({
      ...
      headers: {
        "Accept": isImage ? "image/avif,image/webp,image/apng,image/*,*/*;q=0.8" : isApi ? "application/json, text/plain, */*" : "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
  ...
  var extractEmbeddedJson = (html, id) => {
  ...
  var parseStatus = (status, translationCompleted) => {
    switch (status) {
      case 1:
        return "Ongoing";
      case 2:
        return translationCompleted === false ? "Ongoing" : "Completed";
      case 3:
        return "Dropped";
      case 4:
        return "Hiatus";
  ...
  var parseMangaDetails = (html, mangaId) => {
    ...
    const tags = genreNames.map((name) => App.createTag({ id: name, label: name }));
  ...
  var buildChapterId = (chapter) => `${chapter.hid}-chapter-${chapter.chap}-${chapter.lang}`;
  ...
  var ComickInfo = {
    ...
    sourceTags: [
      { text: "Multi-Language", type: import_types.BadgeColor.BLUE }
    ],
  ...
  var Comick = class extends import_types.Source {
    constructor() {
      super(...arguments);
                ...
                rows: async () => [
                  App.createDUISelect({
                    ...
                    value: App.createDUIBinding({
                      ...
                      set: async (newValue) => await stateManager.store("language", newValue[0] ?? DEFAULT_LANGUAGE)
                    }),
                ...
                rows: async () => [
                  App.createDUISelect({
                    ...
                    value: App.createDUIBinding({
                      ...
                      set: async (newValue) => await stateManager.store("domain", newValue[0] ?? DEFAULT_DOMAIN)
                    }),
                    allowsMultiselect: false
                  })
                ]
...
Please go to the homepage of <${domain}> and press the cloud icon.`);
    ...
    async getHomePageSections(sectionCallback) {
      const sections = [
        ...
        { id: "popular_7", title: "Most Recent Popular (7 days)", path: "/api/comics/top?days=7&type=follow", type: import_types.HomeSectionType.singleRowNormal, more: false },
        { id: "popular_30", title: "Most Recent Popular (1 month)", path: "/api/comics/top?days=30&type=follow", type: import_types.HomeSectionType.singleRowNormal, more: false },
        { id: "popular_90", title: "Most Recent Popular (3 months)", path: "/api/comics/top?days=90&type=follow", type: import_types.HomeSectionType.singleRowNormal, more: false },
        { id: "new_follow_7", title: "Most Followed New Comics (7 days)", path: "/api/comics/top?days=7&type=most_follow_new", type: import_types.HomeSectionType.singleRowNormal, more: false },
        { id: "new_follow_30", title: "Most Followed New Comics (1 month)", path: "/api/comics/top?days=30&type=most_follow_new", type: import_types.HomeSectionType.singleRowNormal, more: false },
        { id: "new_follow_90", title: "Most Followed New Comics (3 months)", path: "/api/comics/top?days=90&type=most_follow_new", type: import_types.HomeSectionType.singleRowNormal, more: false },
        { id: "updates", title: "Updates", path: "/api/chapters/latest?order=new&page=1", type: import_types.HomeSectionType.singleRowNormal, more: true }
      ];
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      return App.createPagedResults({
        results,
        metadata: results.length >= UPDATES_PAGE_SIZE ? { page: page + 1 } : void 0
      });
    }
    async getSearchTags() {
    ...
    async getSearchResults(query, metadata) {
      ...
      return App.createPagedResults({
        results,
        metadata: nextCursor && results.length >= SEARCH_PAGE_SIZE ? { cursor: nextCursor } : void 0
      });
    }
